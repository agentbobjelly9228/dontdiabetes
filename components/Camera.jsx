...
export default function CameraPage({ navigation }) {
    ...
    async function storeData(value, imageLink) {
        try {
            ...
            macros.diary += value.diary;
            macros.numMeals += 1;
            macros.images.push(imageLink)
            macros.GIs.push(value.GIindex)
            // console.log(JSON.parse(value)["fruit"] + "sup")
    ...
    useFocusEffect(
        React.useCallback(() => {
            ...
            return () => {
                // Optional: Reset or perform any cleanup tasks when the screen loses focus
                // For example, you could set cameraOpen back to false
                // setOpen(false);
            };
        }, [])
    );
    ...
    function sendRequest(imageData, imageLink) {
            ...
            "contents": [
                {
                    ...
                    "parts": [
                        {
                            "text": `Here is an image of food. Considering the quantitity of the meal, estimate each of the following quantities: Calories, fruits (cups), vegetables (cups), grains (ounces), protein (ounces), dairy (cups), GI index. Return your answer in only a JSON format like this: 
                            {
                                "food": food description with quanity in ounces,
                                ...
                                "diary": amount of diary in cups,
                                "GIindex": estimated GI index of the food
                            }. consult online sources and be realistic. `
                        },
                        {
                            "inlineData": {
                                "mimeType": "image/jpeg",
        ...
        const config = {
            headers: {
                ...
                "Authorization": `Bearer ya29.a0Ad52N39X9FyMHyqmRNO6rELZCuhbUccC0RRr9ROT-9JKcq0jE97wygSiCmvagXmGMr7vmGlP9cvTAtrj4QqUMkl-qT6Pt7Wx35o-0SzUX4-v0Sm1ql9GQE7u8ICnZJzg8TLeushX1_JCiHR36e1BJKEpIDK3AvHFVSNXEKrfowaCgYKAZ4SARASFQHGX2MiiPkQ1sBubYbuiYx_xoCDxg0177`, // Replace YOUR_ACCESS_TOKEN with your actual access token
    ...
    function sendAway() {
        // Add code for if it's not done
        navigation.navigate("DailyMacros")
    }
